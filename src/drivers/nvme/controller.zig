...
fn toggleController(bar: Pcie.Bar, enable: bool) void {
    var cc = tmp.readRegister(tmp.CCRegister, bar, .cc);
    log.info("CC register before toggle: {}", .{cc});
    cc.en = if (enable) 1 else 0;
    tmp.writeRegister(tmp.CCRegister, bar, .cc, cc);

    cc = tmp.readRegister(tmp.CCRegister, bar, .cc);
    log.info("CC register after toggle: {}", .{cc});

    while (tmp.readRegister(tmp.CSTSRegister, bar, .csts).rdy != @intFromBool(enable)) {}
