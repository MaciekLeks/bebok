...
pub fn createMockPartition(allocator: std.mem.Allocator) !*Partition {
    ...
    mock.* = .{ .alloctr = allocator, .block_device = mdev.mockBlockDevice, .parent = &mdev.mockBlockDevice, .partition_type = Partition.Type.unknown, .attributes = Partition.Attributes{ .required_to_function = false, .type_guid_specific = 0 }, .name = pname };
    return mock;
}
